...
util.getAutoResponses = async (channelId, guildId) => {
  ...
  if (!channelResponses.has(channelId)) {
    if (!await util.refreshChannelResponses(channelId, guildId))
      return [];
  }

  if (!guildResponses.has(guildId)) {
    if (!await util.refreshGuildResponses(guildId))
      return [];
  }
...
util.saveResponse = async (response) => {
  ...
  if (response.global) {
    await util.refreshGuildResponses(response.gid)
  }
  else {
    for (const channel of response.channels) {
      await util.refreshChannelResponses(channel, response.gid);
    }
  }
};

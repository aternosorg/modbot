...
util.log = async (message, logMessage, logEmbed) => {
  ...
  try {
    if (logMessage && logEmbed)
      ...
      await message.guild.channels.resolve(guilds.get(message.guild.id).logChannel).send(logMessage);
    else if (logEmbed)
      await message.guild.channels.resolve(guilds.get(message.guild.id).logChannel).send(new Discord.MessageEmbed(logEmbed));
  } catch (e) {

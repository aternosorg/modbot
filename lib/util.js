...
/**
 * Get a responses for a guild
 * @async
 * @param {GuildInfo} guildInfo
 * @return {AutoResponse}
 */
util.getAutoResponse = async (guildInfo) => {
  let guild = await util.resolveGuild(guildInfo);

  if (!responses.has(guild.id)) {
    if (!await util.refreshResponses(guild.id))
      return [];
  }

  return responses.get(guild.id);
};
...
util.saveResponse = async (response) => {
  await database.query("INSERT INTO responses (response,guildid) VALUES (?,?)",[JSON.stringify(response),response.gid]);
 ...
 */
util.refreshResponses = async (guildId) => {
  const result = await database.queryAll("SELECT * FROM responses WHERE guildid = ?", guildId);
  if(!result.length)
    return false;
  const newResponses = [];
  for (const res of result) {
    newResponses.push(new AutoResponse(res.guildid, JSON.parse(res.response), res.id));
  }
  responses.set(guildId, newResponses);
  setTimeout(() => {
    responses.delete(result.id);
  },cacheDuration);
  return true;
};

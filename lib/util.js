...
util.log = async (message, logMessage, logEmbed) => {
  if (!guilds.has(message.guild.id)) {
    let result = await database.query("SELECT * FROM guilds WHERE id = ?", message.guild.id);
    if(!result)
      return;
  ...
  try {
    ...
    console.error("Couldnt log message", e)
  }
}

util.refreshGuildConfig = async (guildId, database) => {

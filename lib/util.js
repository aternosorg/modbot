...
util.logMessageDeletion = async (message, reason) => {
  util.logMessageEmbed(message, `Message in <#${message.channel.id}> deleted`, {
      footer: {
...
util.log = async (guildInfo, logMessage, logEmbed) => {
  ...
  try {
    if (logMessage && logEmbed)
      await guild.channels.resolve(guildConfig.logChannel).send(logMessage, new Discord.MessageEmbed(logEmbed));
    else if (logMessage)
      await guild.channels.resolve(guildConfig.logChannel).send(logMessage);
    else if (logEmbed)
      await guild.channels.resolve(guildConfig.logChannel).send(new Discord.MessageEmbed(logEmbed));
  } catch (e) {

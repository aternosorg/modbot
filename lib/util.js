...
util.channelMentionToId = (mention) => {
  if(!/^<#\d+>$/.test(mention) && !/^\d+$/.test(mention))
    return null;
...
util.log = async (guildInfo, logMessage, logEmbed) => {

  let guild
  if(guildInfo instanceof Discord.Message)
    guild = guildInfo.guild;
  else
    guild = bot.guilds.resolve(guildInfo);
  ...
  try {
    if (logMessage && logEmbed)
      await guild.channels.resolve(guilds.get(guild.id).logChannel).send(logMessage, new Discord.MessageEmbed(logEmbed));
    else if (logMessage)
      await guild.channels.resolve(guilds.get(guild.id).logChannel).send(logMessage);
    else if (logEmbed)
      await guild.channels.resolve(guilds.get(guild.id).logChannel).send(new Discord.MessageEmbed(logEmbed));
  } catch (e) {
    console.error("Failed to log", e)
  }
}

util.mutedRole = async (guildInfo) => {
  let guild
  if(guildInfo instanceof Discord.Message)
    guild = guildInfo.guild;
  else
  ...
  return guilds.get(guild.id).mutedRole;
}

util.getMute = async (guildInfo) => {
  return await database.query("SELECT * FROM current-moderations WHERE userid = ?, guildid = ?, action = 'muted'");
}

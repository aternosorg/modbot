...
util.getAutoResponses = async (channelId, guildId) => {
  ...
  if (!responses.has(channelId)) {
    if (!await util.refreshResponses(channelId, guildId))
      return [];
  }

  return responses.get(channelId);
};
...
util.saveResponse = async (response) => {
  ...
  await database.query("INSERT INTO responses (`guildid`, `trigger`, `response`, `global`, `channels`) VALUES (?,?,?,?,?)",AutoResponse.destruct(response));
  await util.refreshResponses(response.gid);
};
 ...
 * @return {Boolean} was there a response for this guild
 */
util.refreshResponses = async (channelId, guildId) => {
  const result = await database.queryAll("SELECT * FROM responses WHERE guildid = ? AND global = TRUE OR channels LIKE ?", [guildId,`%${channelId}%`]);
  ...
  for (const res of result) {
    newResponses.set(res.id, new AutoResponse(res.guildid, {
      ...
      global: res.global,
      channels: res.global ? [] : res.channels.split(',')
    }, res.id));
  }
  responses.set(channelId, newResponses);
  setTimeout(() => {
    responses.delete(channelId);
  },cacheDuration);
  return true;
};

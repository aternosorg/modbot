...
util.moderationDBAdd = async (guildId, userId, action, reason, duration, moderatorId) => {
  ...
  if (duration) {
    return await database.queryAll("INSERT INTO moderations (guildid, userid, action, created, expireTime, reason, moderator) VALUES (?,?,?,?,?,?,?)",[guildId, userId, action, now, now + duration, reason, moderatorId]);
  }
  else {
    return await database.queryAll("INSERT INTO moderations (guildid, userid, action, created, reason, moderator) VALUES (?,?,?,?,?,?)",[guildId, userId, action, now, reason, moderatorId]);
  }
}

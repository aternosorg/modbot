 ...
 */
util.getAutoResponses = async (channelId, guildId) => {

  if (!channelResponses.has(channelId)) {
    await util.refreshChannelResponses(channelId);
  }

  if (!guildResponses.has(guildId)) {
    await util.refreshGuildResponses(guildId);
  }
 ...
 * @return {Discord.Collection<Number,AutoResponse>}
 */
util.getAllAutoResponses = async (guildId) => {
...
/**
 * Save response to db and refresh cache
 * @async
 * @param {AutoResponse} response
 * @return {Number} id in db
 */
util.saveResponse = async (response) => {
  if (!response.channels) {response.channels = null;}

  let dbentry = await database.queryAll("INSERT INTO responses (`guildid`, `trigger`, `response`, `global`, `channels`) VALUES (?,?,?,?,?)",response.serialize());

  response.id = dbentry.insertId;

  if (response.global) {
    guildResponses.get(response.gid).set(response.id, response)
  }
  else {
    for (const channel of response.channels) {
      if(!channelResponses.has(channel))
        channelResponses.set(channel, new Discord.Collection());
      channelResponses.get(channel).set(response.id, response)
    }
  }
 ...
 */
util.removeResponse = async (response) => {
  await database.query("DELETE FROM responses WHERE id = ?",[response.id]);

  if (response.global) {
    guildResponses.get(response.gid).delete(response.id)
  }
  else {
    for (const channel of response.channels) {
      channelResponses.get(channel).delete(response.id)
    }
  }
}
 ...
 * @param {Discord.Snowflake} channelId
 */
util.refreshChannelResponses = async (channelId) => {
 ...
 * @param {Discord.Snowflake} guildId
 */
util.refreshGuildResponses = async (guildId) => {
 ...
 */
util.responseEmbed = (response, title, color) => {
  return new Discord.MessageEmbed()
      .setTitle(title + ` [${response.id}]`)
      .setColor(color)
      .addFields([
        {name: "Trigger", value: `${response.trigger.type}: \`${response.trigger.type === 'regex' ? '/' + response.trigger.content + '/' + response.trigger.flags : response.trigger.content}\``},
        {name: "Response", value: response.response.substring(0,1000)},
        {name: "Channels", value: response.global ? "global" : response.channels.map(c => `<#${c}>`).join(', ')}
      ]);
};
